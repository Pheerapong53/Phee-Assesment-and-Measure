...
export default function TablePersonalAttributes() {
  ...
  const columns = [
    ...
    { field: "PersAttrCatFullscore", headerName: "คะแนนเต็ม", width: 300 },
    {
      field: "action",
  ...
  useEffect(() => {
    Axios.get("http://localhost:3001/TableAttr").then((response) => {
  ...
  const toDeleteAttr = (id) => {
    Axios.delete(`http://localhost:3001/deleteAttr/${id}`).then((response) => {
      setAttrList(
        AttrList.filter((val) => {
          return val.id != id;
        })
